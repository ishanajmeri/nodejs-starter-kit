...
const NoFaqsMessage = ({ t }) => <div className="text-center">{t('faqs.noListingsMsg')}</div>;
  ...
  const columns = [
    {
      ...
      dataIndex: 'question',
      key: 'question',
      render: (text, record) => <div>{record.node && record.node.question}</div>
    },
    {
      title: <>{'Is Featured'}</>,
...

    {
      ...
      render: (text, record) => (
        <div>
          <Link className="faqs-link" to={`/edit-faq/${record.node && record.node.id}`}>
